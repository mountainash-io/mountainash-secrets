...
def test_set_then_get_round_trips(tmp_path):
    s = FilesystemSecretStore(tmp_path)
    s.set("svc.acct", {"token": "abc", "n": 3})
    assert s.get("svc.acct") == {"token": "abc", "n": 3}


def test_get_unknown_returns_none(tmp_path):
    assert FilesystemSecretStore(tmp_path).get("nope") is None
...
def test_satisfies_clearable_store(tmp_path):
    assert isinstance(FilesystemSecretStore(tmp_path), ClearableSecretStore)
